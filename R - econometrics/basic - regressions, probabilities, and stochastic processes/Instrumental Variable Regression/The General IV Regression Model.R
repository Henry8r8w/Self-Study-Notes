---
title: "The General IV Regression Model"
...
#### Introduction
The general IV Regression Model can be broken down to four parts: 1. the dependent variable 2. the exogenous variables 3.the endogenous variables 4. the instrumental variables

Using the general form, we can extend from single IV regression model to multiple IV regression model
...
- $\beta_0, \ldots, \beta_{k+1}$: $ 1 + k + r$ unknown regression coefficients.
- $X_{1i}, ..., X_{ki}: k$ endogenous regressors.
- $W_{1i}, ..., W_{ri}: r$ exogenous regressors, uncorrelated with $u_i$.
- $u_i $: the error term.
- $Z_{1i}, ..., Z_{mi} : m $ instrumental variables.

The coefficients are overidentified if $m > k $. If $m < k $, the coefficients are underidentified. When $m = k $ (# of instrument var = # of endgenous var), they are exactly identified. For estimation of the IV regression model, we require exact identification or overidentification.


Assume you want to estimate the model

$$Y_i = \beta_0 + \beta_1 X_{1i} + \beta_2 X_{2i} + W_{1i} + u_i$$
we can input our variables with `ivgreg` using y~ x1 + x2 + w1 |w1 + z1 + z2 + z3

- we have m of 3 with k of 2; # of IV > # of EV
- since w1 is exogenous, we use w1 as a instrument to itself

One shorthand to write IV is .-, which to exclude all endogenous variable in the instrumental variable
...
- The assumptions that allow use to compute t-Test, F-test and Confidence Interval for model coefficients


1. $E(u_i|W_{1i},...,W_{ri}) = 0$
2. $(X_{1i}, ..., X_{ki}; W_{1i}, ..., W_r{i}, Z_{1i},..., Z_{mi})$ are independent and identically distributed draw from their joint distribution
...
However, when it comes to instrument variable, there is a validity issues. One should know how to verify instrument, which it will be discussed in another markdown
